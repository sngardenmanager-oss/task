...
import {
  ...
  LogOut, Menu, MessageCircle, Newspaper, Pause, Play, Plus,
...
type Modal = 'task' | 'note' | 'routine' | 'member' | 'detail' | null;
...
export default function WorkCalendarApp() {
  ...
  const overdue = data.tasks.filter((task) => task.date < today && task.status !== 'completed');
  const todayTasks = data.tasks.filter((task) => task.date === today && task.status !== 'completed');
    ...
    <main className="min-h-screen bg-[#f5f3ec] text-[#26352d]">
      ...
      <div className="mx-auto grid max-w-[1680px] md:grid-cols-[230px_minmax(0,1fr)]">
        ...
        <section className="min-w-0 p-4 pb-24 md:p-7 md:pb-8">
          ...
          {view === 'today' && <TodayView data={data} actor={actor} todayTasks={todayTasks} requests={completionRequests} overdue={overdue} openTask={openTask} navigate={navigate} openCreateTask={openCreateTask} />}
          {view === 'calendar' && <CalendarView data={data} tasks={visibleTasks} month={monthCursor} setMonth={setMonthCursor} openTask={openTask} openCreateTask={openCreateTask} />}
          ...
          {view === 'routines' && <RoutinesView data={data} canEdit={canEdit} openCreate={() => setModal('routine')} updateData={updateData} />}
...
function TodayView({ data, actor, todayTasks, requests, overdue, openTask, navigate, openCreateTask }: { data: WorkspaceState; actor: Member; todayTasks: Task[]; requests: Task[]; overdue: Task[]; openTask: (id:string)=>void; navigate:(view:View)=>void; openCreateTask:(date?:string)=>void }) {
  const active = todayTasks.filter((task)=>task.status!=='completed');
  const note = data.notes.find((item)=>item.date===isoDate());
  return <div className="space-y-5"><div className="grid grid-cols-2 gap-3 xl:grid-cols-4">{[["오늘 업무",String(active.length),"바로 처리할 업무"],["완료 요청",String(requests.length),"관리자 확인 필요"],["지연 업무",String(overdue.length),"마감일 경과"],["이번 주 일정",String(data.tasks.filter((task)=>{const diff=(new Date(task.date).getTime()-new Date(isoDate()).getTime())/86400000;return diff>=0&&diff<=7}).length),"7일 이내 일정"]].map(([label,value,noteText],index)=><button key={label} onClick={()=>navigate(index===1?'notifications':index===2?'tasks':'calendar')} className="rounded-2xl border border-[#d8ded4] bg-[#fbfaf5] p-4 text-left shadow-[0_7px_22px_rgba(55,74,62,0.05)] transition hover:-translate-y-0.5"><div className="flex items-start justify-between"><p className="text-xs font-bold text-[#56645b]">{label}</p>{index===1&&<CheckCircle2 className="size-4 text-[#2f6b4f]"/>}</div><strong className="mt-2 block text-2xl font-black">{value}</strong><p className="mt-1 text-xs text-[#5f6d64]">{noteText}</p></button>)}</div><div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_330px]"><section className="rounded-3xl border border-[#d8ded4] bg-[#fbfaf5] p-4 shadow-[0_12px_36px_rgba(55,74,62,0.06)] sm:p-5"><div className="mb-4 flex items-center justify-between"><div><h3 className="font-black">{actor.name}님의 오늘 업무</h3><p className="text-xs text-[#5f6d64]">완료하면 관리자에게 최종 확인을 요청하세요.</p></div><Button variant="outline" onClick={()=>openCreateTask()}><Plus/>추가</Button></div><div className="space-y-2">{active.length?active.map((task)=><TaskCard key={task.id} task={task} data={data} openTask={openTask}/>):<Empty title="오늘 예정된 업무가 없습니다." action="업무 추가" onClick={()=>openCreateTask()}/>}</div></section><aside className="space-y-4"><section className="rounded-3xl bg-[#2f6b4f] p-5 text-white shadow-[0_14px_34px_rgba(39,91,67,0.2)]"><div className="mb-4 flex items-center justify-between"><h3 className="font-black">이번 주 핵심</h3><span className="rounded-full bg-white/15 px-2 py-1 text-[10px] font-bold">자동 요약</span></div><ul className="space-y-3 text-sm"><li className="flex gap-3"><span className="min-w-10 font-black text-[#f2cf6b]">{requests.length}건</span><span>최종 완료 확인 대기</span></li><li className="flex gap-3"><span className="min-w-10 font-black text-[#f2cf6b]">{overdue.length}건</span><span>마감이 지난 업무</span></li><li className="flex gap-3"><span className="min-w-10 font-black text-[#f2cf6b]">D-7</span><span>가을 프로그램 현장 검토</span></li></ul></section><button onClick={()=>navigate('notes')} className="w-full rounded-3xl border border-[#e3d5ad] bg-[#fff8df] p-5 text-left transition hover:-translate-y-0.5"><div className="mb-3 flex items-center justify-between"><h3 className="font-black">오늘의 특이사항</h3>{note&&<span className="rounded-full bg-[#f0c85a] px-2 py-1 text-[10px] font-black">{note.label.split('-').at(-1)}</span>}</div><p className="text-sm font-bold">{note?.body ?? '등록된 특이사항이 없습니다.'}</p>{note&&<p className="mt-2 text-xs text-[#7b6d43]">{note.location} · {note.urgent?'긴급 확인':'일반'}</p>}</button></aside></div></div>;
}

function CalendarView({ data, tasks, month, setMonth, openTask, openCreateTask }: { data:WorkspaceState; tasks:Task[]; month:Date; setMonth:(date:Date)=>void; openTask:(id:string)=>void; openCreateTask:(date?:string)=>void }) {
  ...
  return <article className="overflow-hidden rounded-3xl border border-[#d8ded4] bg-[#fbfaf5] shadow-[0_12px_36px_rgba(55,74,62,0.06)]"><div className="flex items-center justify-between border-b border-[#e0e4de] px-3 py-4 sm:px-6"><div className="flex items-center gap-1 sm:gap-2"><Button aria-label="이전 달" variant="ghost" size="icon" onClick={()=>setMonth(new Date(year,monthIndex-1,1))}><ChevronLeft/></Button><h3 className="text-base font-black sm:text-lg">{year}년 {monthIndex+1}월</h3><Button aria-label="다음 달" variant="ghost" size="icon" onClick={()=>setMonth(new Date(year,monthIndex+1,1))}><ChevronRight/></Button></div><Button variant="outline" className="rounded-xl bg-white" onClick={()=>setMonth(new Date(`${isoDate().slice(0,7)}-01T00:00:00`))}>오늘</Button></div><div className="grid grid-cols-7 border-b border-[#e0e4de] bg-[#f1f2ed]">{['월','화','수','목','금','토','일'].map(day=><div key={day} className="py-2 text-center text-xs font-bold text-[#5f6d64]">{day}</div>)}</div><div className="grid grid-cols-7">{cells.map((day,index)=>{const date=day?dateKey(day):'';const items=day?tasks.filter((task)=>task.date===date):[];const isToday=date===isoDate();return <div key={index} className={`min-h-24 border-b border-r border-[#e5e7e2] p-1 sm:min-h-32 sm:p-2 ${isToday?'bg-[#edf4ef]':day?'hover:bg-[#f4f5f1]':'bg-[#f3f1eb]/60'}`}>{day&&<><button onClick={()=>openCreateTask(date)} className={`grid size-7 place-items-center rounded-full text-xs font-bold ${isToday?'bg-[#2f6b4f] text-white':'text-[#56635b] hover:bg-[#e2e8e2]'}`}>{day}</button><div className="mt-1 space-y-1">{items.slice(0,3).map(task=>{const category=data.categories.find(item=>item.id===task.categoryId);return <button key={task.id} onClick={()=>openTask(task.id)} className="block w-full truncate rounded-md px-1 py-1 text-left text-[9px] font-black text-[#17231c] sm:text-[11px]" style={{background:category?.color??'#9aa49d'}}>{task.priority==='urgent'?'! ':''}{task.title}</button>})}{items.length>3&&<button onClick={()=>openCreateTask(date)} className="text-[10px] font-bold text-[#526158]">+{items.length-3}개 더보기</button>}</div></>}</div>})}</div></article>;
}
...
function RoutinesView({ data,canEdit,openCreate,updateData }: { data:WorkspaceState;canEdit:boolean;openCreate:()=>void;updateData:(fn:(data:WorkspaceState)=>WorkspaceState,message?:string)=>void }) {
  ...
  return <div className="space-y-4"><div className="flex justify-end">{canEdit&&<Button onClick={openCreate}><Plus/>루틴 추가</Button>}</div><div className="grid gap-3 lg:grid-cols-2">{data.routines.map(routine=>{const category=data.categories.find(item=>item.id===routine.categoryId);const member=data.members.find(item=>item.id===routine.assigneeId);return <article key={routine.id} className={`rounded-3xl border border-[#d8ded4] bg-[#fbfaf5] p-5 ${!routine.active?'opacity-60':''}`}><div className="flex items-start justify-between gap-3"><div><div className="mb-2 flex items-center gap-2"><CategoryDot category={category}/><span className="text-xs font-bold text-[#748078]">{category?.name}</span></div><h3 className="font-black">{routine.title}</h3><p className="mt-1 text-sm text-[#748078]">{routine.cadence} · {member?.name}</p></div><span className={`rounded-full px-2 py-1 text-[10px] font-black ${routine.active?'bg-[#e3eee7] text-[#2f6b4f]':'bg-[#ecebe6] text-[#7b847e]'}`}>{routine.active?'사용 중':'중단됨'}</span></div><div className="mt-4 rounded-xl bg-[#f1f2ed] p-3"><p className="text-xs font-bold text-[#748078]">체크리스트 {routine.checklist.length}개 · 다음 {formatDate(routine.nextDate)}</p>{routine.referenceUrl&&<a href={routine.referenceUrl} target="_blank" rel="noreferrer" className="mt-2 inline-flex items-center gap-1 text-xs font-bold text-[#2f6b4f]"><Link2 className="size-3"/>참고 링크</a>}</div>{canEdit&&<div className="mt-4 flex flex-wrap gap-2"><Button size="sm" variant="outline" onClick={()=>createToday(routine)}><Play/>오늘 실행</Button><Button size="sm" variant="outline" onClick={()=>updateData(current=>({...current,routines:current.routines.map(item=>item.id===routine.id?{...item,active:!item.active}:item)}),routine.active?'루틴을 중단했습니다. 과거 이력은 유지됩니다.':'루틴을 다시 시작했습니다.')} >{routine.active?<Pause/>:<Play/>}{routine.active?'중단':'재시작'}</Button></div>}</article>})}</div></div>;
}
...
function NotesView({ data,canEdit,isAdmin,openCreate,openTask,updateData,actor }: { data:WorkspaceState;canEdit:boolean;isAdmin:boolean;openCreate:()=>void;openTask:(id:string)=>void;updateData:(fn:(data:WorkspaceState)=>WorkspaceState,message?:string)=>void;actor:Member }) {
  ...
  return <div><div className="mb-4 flex justify-end">{canEdit&&<Button onClick={openCreate}><Plus/>특이사항 등록</Button>}</div><div className="space-y-3">{data.notes.map(note=>{const category=data.categories.find(item=>item.id===note.categoryId);const author=data.members.find(member=>member.id===note.createdBy);return <article key={note.id} className={`rounded-3xl border bg-[#fbfaf5] p-5 ${note.urgent?'border-[#e3b5ae]':'border-[#d8ded4]'}`}><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start"><div><div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-[#f0c85a] px-2 py-1 text-[10px] font-black">{note.label}</span>{note.urgent&&<span className="rounded-full bg-[#fae0dc] px-2 py-1 text-[10px] font-black text-[#9b453c]">긴급</span>}<span className="flex items-center gap-1 text-xs font-bold text-[#748078]"><CategoryDot category={category}/>{category?.name}</span></div><h3 className="mt-3 font-black">{note.body}</h3><p className="mt-2 text-sm text-[#748078]">{formatDate(note.date)} · {note.location} · {author?.name}</p></div>{isAdmin&&<Button variant="outline" onClick={()=>convert(note)}>{note.convertedTaskId?<FileText/>:<RefreshCw/>}{note.convertedTaskId?'연결 업무 보기':'업무로 전환'}</Button>}</div></article>})}</div></div>;
}
...
function TaskForm({ data,actor,defaultDate,close,save }: { data:WorkspaceState;actor:Member;defaultDate:string;close:()=>void;save:(task:Task)=>void }) {
  function submit(event:FormSubmitEvent){event.preventDefault();const form=new FormData(event.currentTarget);const checklist=formText(form,'checklist').split('\n').map(text=>text.trim()).filter(Boolean).map(text=>({id:uid('check'),text,done:false}));save({id:uid('task'),title:formText(form,'title'),description:formText(form,'description'),date:formText(form,'date'),endDate:formText(form,'endDate')||undefined,categoryId:formText(form,'category'),assigneeId:formText(form,'assignee'),collaborators:[],priority:formText(form,'priority') as Task['priority'],status:'scheduled',type:formText(form,'type') as Task['type'],checklist,comments:[],createdBy:actor.id,createdAt:new Date().toISOString()});close();}
...
function TaskDetail({ task,data,canEdit,isAdmin,close,toggleChecklist,setTaskStatus,deleteTask,addComment }: { task:Task;data:WorkspaceState;canEdit:boolean;isAdmin:boolean;close:()=>void;toggleChecklist:(taskId:string,checkId:string)=>void;setTaskStatus:(taskId:string,status:Task['status'])=>void;deleteTask:(taskId:string)=>void;addComment:(body:string)=>void }) {
