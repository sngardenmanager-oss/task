    ...
    return (
      <main className="grid min-h-screen place-items-center bg-[#f5f3ec] p-5 text-[#26352d]">
        <section className="w-full max-w-md rounded-3xl border border-[#d8ded4] bg-[#fbfaf5] p-7 text-center shadow-[0_24px_70px_rgba(55,74,62,0.12)]">
          <div
            className={`mx-auto mb-4 grid size-14 place-items-center rounded-2xl ${isPending ? 'bg-[#fff1dd] text-[#806743]' : 'bg-[#f7e8e4] text-[#a83f36]'}`}
          >
            {isPending ? <Clock3 /> : <LockKeyhole />}
          </div>
          <h1 className="text-xl font-black">
            {isPending ? '관리자 승인 대기 중' : '접근 권한이 없습니다'}
          </h1>
          <p className="mt-2 text-sm leading-6 text-[#5f6d64]">
            {gate.message}
          </p>
          <p className="mt-3 rounded-xl bg-white px-3 py-2 text-xs font-semibold text-[#5f6d64]">
            {session.user.email}
          </p>
          {isPending && (
            <button
              ...
              className="mt-5 inline-flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-[#2f6b4f] px-4 text-sm font-bold text-white"
            >
          ...
          <button
            ...
            className={`${isPending ? 'mt-2 border border-[#d8ded4] bg-white text-[#26352d]' : 'mt-5 bg-[#2f6b4f] text-white'} inline-flex h-11 w-full items-center justify-center gap-2 rounded-xl px-4 text-sm font-bold`}
          >
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-[#f5f3ec] text-[#26352d]">
      <div className="flex items-center gap-3 rounded-2xl border border-[#d8ded4] bg-[#fbfaf5] px-5 py-4 text-sm font-bold shadow-sm">
        <LoaderCircle className="size-5 animate-spin text-[#2f6b4f]" />
        {label}
      </div>
    </main>
  );
...
type LoginSubmitEvent = Parameters<
...
function LoginPanel({ supabase }: { supabase: SupabaseClient }) {
  ...
  useEffect(() => {
    void fetch('/api/auth/bootstrap', { cache: 'no-store' })
      .then(
        async (response) => (await response.json()) as { available?: boolean },
      )
      .then((result) => {
        if (result.available) {
  ...
  async function submit(event: LoginSubmitEvent) {
    ...
    try {
      if (mode === 'bootstrap') {
        ...
        if (!response.ok)
          throw new Error(result.error ?? '관리자 계정을 만들지 못했습니다.');
    ...
    <main className="grid min-h-screen bg-[#f5f3ec] p-5 text-[#26352d] lg:grid-cols-[1.1fr_0.9fr]">
      <section className="hidden items-center justify-center rounded-[2rem] bg-[#2f6b4f] p-12 text-white lg:flex">
        <div className="max-w-lg">
          <div className="mb-8 grid size-16 place-items-center rounded-3xl bg-white/15">
          ...
          <p className="mt-5 max-w-md text-sm leading-7 text-white/75">
            업무, 루틴, 현장 특이사항을 안전하게 공유하고 팀의 일정을 한곳에서
            확인하세요.
          </p>
          <div className="mt-10 flex items-center gap-3 rounded-2xl bg-white/10 px-4 py-3 text-sm font-semibold text-white/85">
            <ShieldCheck className="size-5" /> Supabase Auth로 계정과 세션을
            보호합니다.
          </div>
        </div>
      </section>
      ...
      <section className="flex items-center justify-center p-0 sm:p-8">
        <div className="w-full max-w-md rounded-[2rem] border border-[#d8ded4] bg-[#fbfaf5] p-6 shadow-[0_24px_70px_rgba(55,74,62,0.12)] sm:p-8">
          <div className="mb-6 flex items-center gap-3 lg:hidden">
            <div className="grid size-11 place-items-center rounded-2xl bg-[#2f6b4f] text-white">
              <Leaf className="size-5" />
            </div>
            <div>
              <p className="text-[11px] font-bold tracking-[0.12em] text-[#708076]">
                SNOOPY GARDEN
              </p>
              <p className="font-black">워크 캘린더</p>
            </div>
          </div>
          ...
          <form onSubmit={submit} className="mt-7 space-y-4">
            ...
            {error && (
              <p
                role="alert"
                className="rounded-xl bg-[#f7e8e4] px-3 py-2.5 text-sm font-semibold text-[#8d342e]"
              >
                {error}
              </p>
            )}
            {notice && (
              <output className="block rounded-xl bg-[#e7f0eb] px-3 py-2.5 text-sm font-semibold text-[#2f6b4f]">
                {notice}
              </output>
            )}
            <button
              disabled={busy}
              className="inline-flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#2f6b4f] px-4 text-sm font-black text-white transition hover:bg-[#255940] disabled:cursor-wait disabled:opacity-60"
            >
          ...
          <div className="mt-5 flex flex-wrap justify-center gap-x-4 gap-y-2 text-xs font-bold text-[#5f6d64]">
            ...
            {!bootstrapAvailable && mode !== 'signup' && (
              <button
                onClick={() => {
                  setMode('signup');
                  setError('');
...
const authInputClass =
  'mt-1.5 h-12 w-full rounded-xl border border-[#d8ded4] bg-white px-3 text-sm outline-none transition focus:border-[#2f6b4f] focus:ring-2 focus:ring-[#2f6b4f]/10';
...
function AuthField({
  ...
  return (
    <label className="block text-sm font-bold text-[#3e4d44]">
      {label}
      {children}
    </label>
  );
